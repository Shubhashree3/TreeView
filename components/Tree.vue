      ...
      <li :id="node.id" v-for="node in dataTree">
        <div class="treeview__level" data-level="A">
          <span class="level-title">
            <b-button variant="light" @click="toggle(node.id)">{{node.label}}</b-button>
          </span>
          <div class="treeview__level-btns">
            <b-button variant="outline-danger" class="deleteButton" @click="deleteNode(node.id)">
              <FontAwesomeIcon :icon="['fas', 'trash']" />
            </b-button>
            <b-button variant="outline-primary" class="editButton" @click="showModal(node)">
...
.treeview > div > ul > li > .treeview__level::before {
  height: 60px;
  width: 60px;
  top: -9.5px;
